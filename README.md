...
| [`examples/painter/`](./examples/painter) | Ferramenta de pintura livre na página (pincel com pressão, balde de tinta, formas básicas) e transformações de imagem (girar, espelhar, redimensionar, desfocar). Exemplo de referência simples e direto pra `studio.draw` (`stroke`/`floodFill`/`shape`/`blank`) e as funções novas de `studio.image` (`resize`/`rotate`/`flip`/`blur`) - sem nenhum modelo de IA envolvido. |
